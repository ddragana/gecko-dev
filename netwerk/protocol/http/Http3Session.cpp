...
nsresult Http3Session::ProcessInput() {
  ...
  uint32_t read = 0;
  nsresult rv = mSegmentWriter->OnWriteSegment((char*)packet, UDP_MAX_PACKET_SIZE, &read);
  if (NS_FAILED(rv)) {
    return rv;
  }
...
void Http3Session::SetupTimer(uint64_t aTimeout) {
  LOG(("Http3Session::SetupTimer to 0x%" PRIu64 "ms [this=%p].",
       aTimeout, this));
...
nsresult Http3Session::SendRequestBody(uint64_t aStreamId, const char* buf,
  ...
  return mHttp3Connection->SendRequestBody(aStreamId, (const uint8_t*)buf, count, countRead);
}
...
void Http3Session::CallCertVerification() {
  ...
  SECStatus rv = AuthCertificateHookWithInfo(mSocketControl,
...
void Http3Session::Authenticated(int32_t aError) {
  LOG(("Http3Session::Authenticated error=0x%" PRIx32 " [this=%p].",
       aError, this));
  if (mState == INITIALIZING) {
