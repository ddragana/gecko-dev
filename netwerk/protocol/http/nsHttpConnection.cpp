...
bool nsHttpConnection::EnsureNPNComplete(nsresult& aOut0RTTWriteHandshakeValue,
  ...
  if (mHttp3Session) {
    // Use Http3 veersion of EnsureNPNComplete.
...
nsresult nsHttpConnection::Activate(nsAHttpTransaction* trans, uint32_t caps,
  ...
  mPriority = pri;
  if (mTransaction && ((mUsingSpdyVersion != SpdyVersion::NONE) || mHttp3Session)) {
  ...
  if (!mHttp3Session) {
    // Http3 does not need TCP keepalive.
...
void nsHttpConnection::SetupSSL() {
  ...
  // For Http3 ssl is setup when Http3Session has been created.
...
uint32_t nsHttpConnection::ReadTimeoutTick(PRIntervalTime now) {
  ...
  if (mHttp3Session && mNPNComplete) {
    // During tls handaske use nsHttpConnection timeouts, afterwards
    // Http3Session will handle connection timeouts on its own.
  ...
  do {
    ...
    if (mConnInfo->UsingHttpsProxy() &&
      ...
      if (NS_SUCCEEDED(rv) && !transactionBytes &&
          NS_SUCCEEDED(mSocketOutCondition) && !mHttp3Session) {
        mSocketOutCondition = NS_BASE_STREAM_WOULD_BLOCK;
      }
...
nsresult nsHttpConnection::OnSocketReadable() {
  ...
  if (mHttp3Session && !mNPNComplete) {
    // We are still doing handshake. TLS/TCP change pall flags therefore they
    // only trigger OnSocketWritable. Http3/Quic do not do that so imitate
    // behavior here.
    return OnSocketWritable();
  }
...
void nsHttpConnection::CheckForTraffic(bool check) {
  if (check) {
    ...
    if (mSpdySession) {
      ...
      // If not SPDY/Http3, Store snapshot amount of data right now
