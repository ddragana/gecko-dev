...
void Http3Stream::FindRequestContentLength() {
  ...
  // Find the beginning.
  int32_t valueIndex = mFlatHttpRequestHeaders.Find(":", false, contentLengthStart) + 1;
...
nsresult Http3Stream::OnReadSegment(const char* buf, uint32_t count,
  ...
  LOG(("Http3Stream::OnReadSegment count=%u state=%d [this=%p]",
       count, mState, this));
    ...
    case SENDING_BODY:
      {
        ...
        if (NS_FAILED(rv)) {
          LOG3(("Http3Stream::OnReadSegment %p sending body returns error=0x%" PRIx32
                ".", this, static_cast<uint32_t>(rv)));
          return rv;
        }
      ...
      break;
    case EARLY_RESPONSE:
      // We do not need to send the rest of tthe request, so just ignore it.
      *countRead = count;
    ...
    case READING_HEADERS:
      {
        if (mFlatResponseHeaders.IsEmpty()) {
          nsresult rv = mSession->ReadResponseHeaders(mStreamId, &mFlatResponseHeaders, &mFin);
