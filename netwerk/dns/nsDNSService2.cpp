...
nsDNSAsyncRequest::Cancel(nsresult reason)
{
    ...
    mResolver->DetachCallback(mHost.get(), mType, mOriginAttributes, mFlags,
                              mAF, this, reason);
    return NS_OK;
}
                                   ...
                                   nsICancelable          **result)
{
    ...
    rv = res->ResolveHost(req->mHost.get(), type, req->mOriginAttributes, flags, af, req);
    req.forget(result);
    return rv;
}
                                         ...
                                         const OriginAttributes &aOriginAttributes)
{
    ...
    res->CancelAsyncRequest(hostname.get(), aType, aOriginAttributes, aFlags,
                              ...
                              nsIDNSRecord           **result)
{
    ...
    rv = res->ResolveHost(hostname.get(), 0, aOriginAttributes, flags, af, syncReq);
    if (NS_SUCCEEDED(rv)) {
        // wait for result
        while (!syncReq->mDone) {
