...
#define CERTDIR "/home/dragana/dragana_work/sdt/proxycert"
...
int main()
{
  ...
  while (1) {
    ...
    if ((pollElem[0].out_flags & PR_POLL_READ)) {
      ...
      if (flow < 0) {
        // remove packet.
        PR_RecvFrom(udp, udpBuf, SDT_CLEARTEXTPAYLOADSIZE, 0, &peerAddr, PR_INTERVAL_NO_TIMEOUT);
      } else {
