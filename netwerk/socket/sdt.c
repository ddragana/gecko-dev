...
struct sdt_t *sdt_newHandle()
{
  ...
  handle->hIncomingHeaders->mFrames = nullptr;
  ...
  handle->hIncomingHeaders->mNext = nullptr;
...
static int32_t
sLayerSendTo(PRFileDesc *fd, const void *buf, int32_t amount,
...
hOrderHeaderFrame(struct sdt_t *handle, struct hFrame_t *frame)
{
  ...
  while (handle->hIncomingHeaders->mFrames &&
    ...
    if ((streamId > 3) && (streamId % 2)) {
      streamId -= 2;
    }

    struct hStream_t* stream = nullptr;
  ...
  while ((read < amount) && frame) {
    ...
    if (frameAlreadyRead == frameSize) {
      ...
      if (!(flags & PR_MSG_PEEK)) {
        ...
        if (!handle->hOrderedFramesFirst) {
          handle->hOrderedFramesLast = nullptr;
        }
        free(done);
      }
    }
    ...
    switch (handle->hState) {
      case SDT_H2S_NEWFRAME:
      {
        ...
        *read = *read + ((handle->hPadding) ? 1 : 0);
        ...
        if ((http2StreamId > 2) && (http2StreamId % 2)) {
          handle->hSDTStreamId += 2;
        }
