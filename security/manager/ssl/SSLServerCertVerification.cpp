...
class NotifyCertProblemRunnable : public SyncRunnableBase {
 public:
  NotifyCertProblemRunnable(uint64_t fdForLogging, TransportSecurityInfo* infoObject)
...
void NotifyCertProblemRunnable::CallNotifyCertProblem() {
  ...
  if (!NS_IsMainThread()) {
    NS_ERROR(
        "NotifyCertProblemRunnable::CallNotifyCertProblem called off main"
        " thread");
    return;
  }
...
// execute CallNotifyCertProblem on the main thread. CallNotifyCertProblem must
// block events on both of these threads because it calls
...
SECStatus AuthCertificate(CertVerifier& certVerifier,
                          ...
                          uint32_t providerFlags, Time time, uint32_t certVerifierFlags) {
...
PRErrorCode AuthCertificateParseResults(
    ...
    const OriginAttributes& aOriginAttributes, TimeStamp aJobStartTime,
  ...
  if (!aCollectedErrors) {
    ...
    // errors.
    MOZ_LOG(gPIPNSSLog, LogLevel::Debug,
            ("[0x%" PRIx64 "] !aCollectedErrors: %d\n", aPtrForLog,
             static_cast<int>(aDefaultErrorCodeToReport)));
  ...
  if (overrideAllowed) {
    ...
    uint32_t remaining_display_errors = aCollectedErrors;
    ...
    if (overrideService) {
      ...
      nsresult rv = overrideService->HasMatchingOverride(
          aHostName, aPort, static_cast<nsIX509Cert*>(nssCert), &overrideBits,
...
SSLServerCertVerificationJob::Run() {
  ...
  PRErrorCode finalError = AuthCertificateParseResults(
      ...
      mInfoObject->GetOriginAttributes(), jobStartTime, mCert, mProviderFlags,
      mPRTime, error, collectedErrors);
...
SECStatus MakeCertChain(nsTArray<nsTArray<uint8_t>>& inPeerCertChain,
    UniqueCERTCertificate &outCert, UniqueCERTCertList &outCertChain) {
  for (auto& certDer : inPeerCertChain) {
    ...
    if (!outCert) {
      outCert.reset(CERT_NewTempCertificate(
          CERT_GetDefaultCertDB(), &der, nullptr, false, true));
      if (!outCert) {
        MOZ_LOG(gPIPNSSLog, LogLevel::Debug,
                ("MakeCertChain: cert failed"));
        return SECFailure;
      }
    }
    ...
    if (CERT_AddCertToListTail(outCertChain.get(), tmpCert.get()) != SECSuccess) {
      MOZ_LOG(gPIPNSSLog, LogLevel::Debug,
